...
function maint_device_action_execute($action) {
    if ($action == 'maint') {
        ...
        if ($schedule_id && isset($_POST['selected_items'])) {
            $selected = @unserialize(stripslashes($_POST['selected_items']));
        ...
        if ($schedule_id <= 0 || !db_fetch_cell_prepared('SELECT id FROM plugin_maint_schedules WHERE id = ?', array($schedule_id))) {
            return false;
        }
        ...
        $sname = db_fetch_cell_prepared('SELECT name FROM plugin_maint_schedules WHERE id = ?', array($schedule_id));
        if ($sname === null || $sname === '') { $sname = '#' . $schedule_id; }
