...
function schedule_edit(): void {
	...
	} elseif (get_request_var('tab') == 'servcheck') {
		servcheck_test($header_label);
	} else {
...
function thold_hosts(string $header_label): void {
	...
	if (get_request_var('site_id') != MAINT_HOST_FILTER_ANY) {
		...
		$sql_where_params = array_merge($sql_where_params, [get_request_var('site_id')]);
	}

	if (get_request_var('poller_id') != MAINT_HOST_FILTER_ANY) {
		$sql_where .= ' AND poller_id = ?';
		$sql_where_params = array_merge($sql_where_params, [get_request_var('poller_id')]);
	}
	...
	if (get_request_var('associated') == 'false') {
		...
		$sql_where_params = array_merge($sql_where_params, [get_request_var('id')]);
	}
	...
	if (get_request_var('location') != MAINT_HOST_FILTER_LOC_ANY) {
		if (get_request_var('location') == MAINT_HOST_FILTER_LOC_NONE) {
			...
			$sql_where_params = array_merge($sql_where_params, [get_request_var('location')]);
		}
	}
	...
	if (strlen(get_request_var('filter'))) {
		...
		$sql_where_params = array_merge(
			$sql_where_params,
			['%' . get_request_var('filter') . '%', '%' . get_request_var('filter') . '%'],
		);
	}
	...
	if (get_request_var('host_template_id') == MAINT_HOST_FILTER_ANY) {
		...
		$sql_where_params = array_merge($sql_where_params, [get_request_var('host_template_id')]);
	}
	...
	if (get_request_var('associated') == 'false') {
		...
		$sql_where_params = array_merge($sql_where_params, [MAINT_HOST_TYPE_HOSTS, get_request_var('id')]);
	}
	...
	if ($schedule_created) {
		...
		$sql_params = array_merge([get_request_var('id'), get_request_var('id')], $sql_where_params);
...
function webseer_urls(string $header_label): void {
	...
	if ($schedule_created) {
		...
		$sql_params = array_merge([get_request_var('id'), MAINT_HOST_TYPE_WEBSEER, get_request_var('id')], $sql_where_params);
		$sql_query  = "SELECT u.*,
	...
	if (cacti_sizeof($urls)) {
		foreach ($urls as $url) {
			...
			$lists = db_fetch_assoc_prepared(
				...
				[MAINT_HOST_TYPE_WEBSEER, $url['id'], get_request_var('id')],
			);
...
function servcheck_test(string $header_label): void {
	...
	validate_store_request_vars($filters, 'sess_maint_sch');
	...
	<tr class='even'>
		<td>
		<form id='form_servcheck' action='maint.php?action=edit&tab=servcheck'>
			<table class='filterTable'>
				<tr>
					<td>
						<?php print __('Search', 'maint'); ?>
	...
	if ($schedule_created) {
		...
		$sql_params = array_merge([get_request_var('id'), MAINT_HOST_TYPE_SERVCHECK, get_request_var('id')], $sql_where_params);
		$sql_query  = "SELECT t.*,
			(SELECT schedule FROM plugin_maint_hosts WHERE host = t.id AND schedule = ?) AS associated,
			pmh.type AS maint_type
			FROM plugin_servcheck_test AS t
			LEFT JOIN plugin_maint_hosts AS pmh
				ON (t.id = pmh.host
		...
		$tests = db_fetch_assoc_prepared($sql_query, $sql_params);
	} else {
		// new schedule, no "id" yet
		$total_rows = 0;
		$tests      = [];
	...
	if (cacti_sizeof($tests)) {
		foreach ($tests as $test) {
			...
			$lists = db_fetch_assoc_prepared(
				...
				AND host = ?
				AND plugin_maint_schedules.id != ?',
				[MAINT_HOST_TYPE_SERVCHECK, $test['id'], get_request_var('id')],
			);
			...
			if ($names == '') {
				form_selectable_cell('<span class="deviceUnknown">' . __('No Schedules', 'maint') . '</span>', $test['id']);
			} else {
				form_selectable_cell($names, $test['id']);
			}
		...
		if ($schedule_created) {
			print "<tr><td colspan='8'><em>" . __('No Associated Servcheck Test\'s Found', 'maint') . '</em></td></tr>';
		} else {
			print "<tr><td colspan='8'><em>" . __('Schedule must be created before associating', 'maint') . '</em></td></tr>';
		}
	}
	...
	form_hidden_box('id', get_request_var('id'), '');
	form_hidden_box('save_servcheck', '1', '');
