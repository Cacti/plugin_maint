 ...
 * Test bootstrap.
 *
 * Maint's sources expect to be included by Cacti, which has already
 ...
 * $GLOBALS['__test_db_calls'] and hands back a safe default, or the next
 * value queued via maint_test_queue() when a Unit test needs to control it.
 *
 ...
 * maint's source files build include paths from it at runtime.
 */
...
$GLOBALS['__test_stub_queue'] = array();
$GLOBALS['__test_log']        = array();

/**
 * Queue a return value for the next call made to a stubbed Cacti function.
 *
 * Unit tests use this to control what the plugin's db_* calls see, since the
 * stubs below are shared, already-declared functions and cannot be
 * redeclared per test.
 *
 ...
 * @return void
 */
function maint_test_queue($fn, $value) {
	$GLOBALS['__test_stub_queue'][$fn][] = $value;
}

/**
 * Take the next queued return value for a stubbed function, or $default if
 * nothing is queued.
 *
 ...
 * @return mixed
 */
function maint_test_next($fn, $default) {
	if (!empty($GLOBALS['__test_stub_queue'][$fn])) {
		return array_shift($GLOBALS['__test_stub_queue'][$fn]);
	}
...
if (!function_exists('db_execute')) {
	function db_execute($sql) {
		...
		return maint_test_next('db_execute', true);
	}
}
...
if (!function_exists('db_execute_prepared')) {
	function db_execute_prepared($sql, $params = array()) {
		...
		return maint_test_next('db_execute_prepared', true);
	}
}
...
if (!function_exists('db_fetch_assoc')) {
	function db_fetch_assoc($sql) {
		...
		return maint_test_next('db_fetch_assoc', array());
	}
}
...
if (!function_exists('db_fetch_assoc_prepared')) {
	function db_fetch_assoc_prepared($sql, $params = array()) {
		...
		return maint_test_next('db_fetch_assoc_prepared', array());
	}
}
...
if (!function_exists('db_fetch_row')) {
	function db_fetch_row($sql) {
		...
		return maint_test_next('db_fetch_row', array());
	}
}
...
if (!function_exists('db_fetch_row_prepared')) {
	function db_fetch_row_prepared($sql, $params = array()) {
		...
		return maint_test_next('db_fetch_row_prepared', array());
	}
}

if (!function_exists('db_fetch_cell')) {
	function db_fetch_cell($sql) {
		return maint_test_next('db_fetch_cell', '');
	}
}

if (!function_exists('db_fetch_cell_prepared')) {
	function db_fetch_cell_prepared($sql, $params = array()) {
		return maint_test_next('db_fetch_cell_prepared', '');
	}
}
...
if (!function_exists('read_config_option')) {
	function read_config_option($name, $force = false) {
		return maint_test_next('read_config_option', '');
	}
}
...
if (!function_exists('html_escape_request_var')) {
	function html_escape_request_var($name) {
		return htmlspecialchars(get_request_var($name), ENT_QUOTES | ENT_HTML5, 'UTF-8');
	}
}
...
if (!function_exists('get_request_var')) {
	function get_request_var($name) {
		return maint_test_next('get_request_var:' . $name, '');
	}
}

if (!function_exists('get_nfilter_request_var')) {
	function get_nfilter_request_var($name) {
		return maint_test_next('get_nfilter_request_var:' . $name, '');
	}
}

if (!function_exists('get_filter_request_var')) {
	function get_filter_request_var($name) {
		return maint_test_next('get_filter_request_var:' . $name, '');
	}
}
